...
// creates a predicate to decide what color of pixel to create
pix_trans_cb_t build_transform_callback(color const& letter, color const& surface)
{
	return [&](ref_t const& src)
	{
		color c = WHITE;

		switch (to_bits32(src))
		{
		case SRC_EDGE_COLOR:
			c = BLACK;
			break;

		case SRC_LETTER_COLOR:
			c = letter;
			break;

		case SRC_SURFACE_COLOR:
			c = surface;
			break;

		case SRC_BACKGROUND_COLOR:
			c = GREY;
			break;
...
// reads a list of images from disk
std::vector<image_t> read_src_images(std::vector<const char*> const& src_paths)
{
	const gil::image_read_settings<gil::png_tag> read_settings;
	auto const read_img = [&](const char* img_path)
	{
		image_t src_img;
	...
	src_images.reserve(src_paths.size());
	std::transform(src_paths.begin(), src_paths.end(), std::back_inserter(src_images), read_img);

	return src_images;
}


// applies transform and writes image
void build_image(image_t& src_img, const char* dst_path, pix_trans_cb_t const& trans_cb)
{
	image_t dst_img(src_img.dimensions());
	auto dst_v = gil::view(dst_img);

	gil::transform_pixels(gil::view(src_img), dst_v, trans_cb);

	gil::write_view(dst_path, dst_v, gil::png_tag());
}


// builds images using the letters provided
void build_images(std::vector<const char*> const& src_paths, const char* out_dir)
{
	// get max length of index number
	const auto num_images = src_paths.size() * SURFACE_COLRS.size() * LETTER_COLORS.size();
	const auto idx_len = num_digits(num_images);
	unsigned idx = 1;
	char idx_str[100];
	const auto out_path_base = std::string(out_dir);

	for (auto& src : read_src_images(src_paths))
	{
		for (auto& sc : SURFACE_COLRS)
		{
			for (auto& lc : LETTER_COLORS)
			{
				sprintf_s(idx_str, "%0*d", (int)idx_len, idx++); // zero pad index number
				auto out_path = str_append_sub(out_path_base, std::string("block_") + idx_str + ".png");
				auto cb = build_transform_callback(lc, sc);
				build_image(src, out_path.c_str(), cb);
			}
		}
	}
}
